...
export function MissionControlFooter({
  ...
  return (
    <footer className="overflow-hidden rounded-2xl border border-purple-500/20 bg-black/40 backdrop-blur">
        ...
        <div className="relative min-w-0 flex-1 overflow-hidden">
          <div className="animate-marquee flex whitespace-nowrap">
            {tickerItems.map((msg, i) => (
              <span key={`${msg}-${i}`} className="mx-8 text-sm text-muted-foreground">
                {msg}
              </span>
            ))}
          </div>
